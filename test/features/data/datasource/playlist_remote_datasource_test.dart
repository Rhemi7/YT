...
void main() {
  ...
  var nextPlaylistVideos = Uri.parse(
      '$baseUrl/playlistItems?pageToken=$nextPlaylistVidToken&playlistId=$playlistId&key=$apIkey&part=snippet&maxResults=10');
  ...
  group('Get Next Set of Playlist Videos', () {
    test('returns a playlist video response if the http call completes successfully',
            () async {
          when(client.get(nextPlaylistVideos, headers: headers)).thenAnswer((_) async =>
              http.Response(dataReader('playlist_videos_response.json'), 200));

          expect(await dataSource.getNextPlayListVideos(pageToken: nextPlaylistVidToken, playlistID: playlistId), isA<PlaylistVideoResponse>());
        });

    test('throws an exception if the http call results in an error', () {
      when(client.get(nextPlaylistVideos, headers: headers))
          .thenAnswer((_) async => http.Response('Not Found', 404));

      expect(() => dataSource.getNextPlayListVideos(pageToken: nextPlaylistVidToken, playlistID: playlistId),
          throwsA(const TypeMatcher<ServerException>()));
    });
  });
}
